...
async function runPipeline({
  ...
  try {
    ...
    // ── Step 3: Fallback if no files generated ───────────────
    ...
    const hasFiles = latestFragment.files && latestFragment.files.length > 0
    const hasCode = typeof latestFragment.code === 'string' && latestFragment.code.length > 10

    if (!hasFiles && !hasCode) {
      console.log('[Agentic] No files generated by agents, running fallback generation')
      emitAction('commentary', 'Agents did not generate files. Falling back to direct generation...')
      emitAction('status', 'Generating code (fallback)...')
      try {
        const fallbackFragment = await generateFallback(
          messages, model, config, template, supabaseContext
        )
        if (fallbackFragment) {
          // Merge fallback into latestFragment
          if (fallbackFragment.files && fallbackFragment.files.length > 0) {
            latestFragment.files = fallbackFragment.files
          }
          if (fallbackFragment.code) latestFragment.code = fallbackFragment.code
          if (fallbackFragment.file_path) latestFragment.file_path = fallbackFragment.file_path
          if (fallbackFragment.title) latestFragment.title = fallbackFragment.title
          if (fallbackFragment.template && fallbackFragment.template !== 'default') {
            latestFragment.template = fallbackFragment.template
          }
          emitFragment(latestFragment)
          emitAction('commentary', `Fallback generated ${(fallbackFragment.files || []).length || 1} file(s)`)
        }
      } catch (fallbackError) {
...
async function generateFallback(
  ...
  for (const candidate of fallbackChain) {
    try {
      const modelClient = getModelClient(candidate, config)
      console.log(`[Fallback] Trying ${candidate.id} (${candidate.providerId})...`)
      const fallbackAbort = AbortSignal.timeout(60000)
      let text: string

      const result = streamText({
        model: modelClient as any,
        system: systemPrompt + '\n\nYou MUST respond with ONLY a valid JSON object containing files, code, commentary, template, title, description. No markdown explanation outside the JSON.',
        messages,
        maxRetries: 0,
      ...
      if (!text || text.length < 20) {
        console.warn(`[Fallback] ${candidate.id} returned empty response`)
        continue
      }
      ...
      if (json) {
        ...
        if (result.success) {
          console.log(`[Fallback] ${candidate.id} produced valid fragment with ${(result.data as any).files?.length || 0} files`)
          return result.data as any
        }
        const filled = fillDefaults(parsed)
        console.log(`[Fallback] ${candidate.id} produced filled fragment with ${(filled.files || []).length} files`)
        return filled
      }

      // No JSON found — use raw text as code
      console.warn(`[Fallback] ${candidate.id} returned no JSON, using raw text as code`)
      return {
      ...
      console.error(`[Fallback] ${candidate.id} failed:`, error?.message || error)
      continue
    }
  }
